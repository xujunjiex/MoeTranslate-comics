...
**按需下载，优化包体积：**
- PP-OCRv5 核心模型（det + cls + rec_zh + 字典）内置在 APK 中
- 可选 rec 语言模型：rec_en（~7.5MB）、rec_ko（~12.9MB）、rec_ru（~7.7MB）
...
**本地离线翻译：**
- **ML Kit 翻译** — 首次下载模型（~100MB）后可离线使用
- **NLLB 翻译** — 首次下载模型（~1GB）后可离线使用，自动检测设备 RAM（< 6GB 警告）
- **LlamaCpp 引擎** — 可导入任意 GGUF 格式模型，完全离线运行
...
| 方式 | 链接 |
|------|------|
| GitHub Releases | [最新版本 v0.9.0](https://github.com/xujunjiex/StarFlow/releases/tag/v0.9.0) |
...
- 后台批量漫画翻译和打包下载
- 测试本地离线翻译功能
- ~~添加更快、精度更高的 OCR 模型~~（✅ PP-OCRv6 已集成）
- ~~优化当前专用 OCR 模型的响应速度，替换使用最新版 PP-OCRv6 模型~~（✅ 已集成 small 内置 + medium 可选下载）
